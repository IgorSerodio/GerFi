...
export async function getTimelineData(filters: ReportFiltersDTO): Promise<TimelineTicket[]> {
  ...
  let queryStr = `
    SELECT 
      t.id, u.name as attendant, tw.name as guiche, tw.alias as guiche_alias, l.name as location_name, t.priority, t.status, t.ticket_number,
  ...
  return rows.map((row) => ({
    ...
    guiche: (row.guiche_alias || row.guiche) ? (row.location_name ? `${row.location_name} - ${row.guiche_alias || row.guiche}` : (row.guiche_alias || row.guiche)) : "-",
...
export async function getAnalyticalData(filters: ReportFiltersDTO, page: number = 1, limit: number = 50): Promise<{ data: AnalyticalTicket[], total: number }> {
  ...
  const queryStr = `
    SELECT 
      t.*, u.name as attendant, tw.name as guiche, tw.alias as guiche_alias, l.name as location_name,
      (SELECT MIN(created_at) FROM tickets f WHERE f.ticket_number = t.ticket_number AND f.created_at::date = t.created_at::date) as original_created_at,
  ...
  return {
    ...
    data: rows.map((row) => ({
      ticketNumber: row.ticket_number,
      guiche: (row.guiche_alias || row.guiche) ? (row.location_name ? `${row.location_name} - ${row.guiche_alias || row.guiche}` : (row.guiche_alias || row.guiche)) : null,
